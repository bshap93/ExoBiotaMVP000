...
{
    // [WIP] Base class for tools that use the digger system
    public abstract class BaseDiggerUsingTool : MonoBehaviour
    {
        ...
        public virtual Sprite GetToolIcon()
        {
            if (toolIcon == null)
            {
                Debug.LogWarning("Tool icon is not set for BaseDiggerUsingTool.");
                return null;
            }
        ...
        public int GetCurrentTextureIndex()
        {
            try
            {
                ...
                {
                    if (debugLogging) Debug.Log($"Invalid texture index: {forwardTextureIndex}");
                    return 0; // Default safe value
                }

                if (TerrainManager.Instance == null)
                {
                    Debug.LogError("TerrainManager.Instance is null");
                    return 0; // Default safe value
                }

                if (TerrainManager.Instance.currentTerrainController == null)
                {
                    Debug.LogError("Current Terrain Controller is null");
                    return 0; // Default safe value
                }

                var terrainBehavior = TerrainManager.Instance.currentTerrainController.terrainBehavior;
                if (terrainBehavior == null)
                {
                    Debug.LogError("TerrainBehavior is null");
                    return 0; // Default safe value
                }
                ...
                {
                    Debug.LogError($"Invalid dig depth: {digDepth}");
            ...
            {
                Debug.LogError($"Error in GetCurrentTextureIndex: {ex.Message}");
                return 0; // Default safe value
            }
        }
        ...
        protected int GetTerrainLayerBasedOnDepthAndOverrides(int rawTextureIndex, float digDepth)
        {
            try
            {
                ...
                {
                    if (debugLogging) Debug.Log($"Invalid raw texture index: {rawTextureIndex}");
                    return 0; // Default safe value
                }
                ...
                if (currentTerrainBehavior.defaultLayerAboveDepths == null)
                {
                    Debug.LogError("defaultLayerAboveDepths is null");
                    return 0; // Default safe value
                }

                foreach (var defaultLayer in currentTerrainBehavior.defaultLayerAboveDepths)
                {
                    if (defaultLayer == null) continue;
            ...
            {
                Debug.LogError($"Error in GetTerrainLayerBasedOnDepthAndOverrides: {ex.Message}");
                return 0; // Default safe value
            }
        }
        ...
        private int ConsiderTerrainChoices(int forwardTextureIndex)
        {
            try
            {
                if (forwardTextureIndex < 0)
                {
                    Debug.LogError($"Invalid forward texture index: {forwardTextureIndex}");
                    return 0; // Default safe value
                }
                ...
                {
                    Debug.LogError("TerrainManager.Instance is null");
                    return forwardTextureIndex; // Default safe value
                }
                ...
                if (currentTerrainBehavior == null || currentTerrainBehavior.terrainChoices == null)
                    return forwardTextureIndex;

                foreach (var terrainChoice in currentTerrainBehavior.terrainChoices)
                {
            ...
            {
                Debug.LogError($"Error in ConsiderTerrainChoices: {ex.Message}");
                return 0; // Default safe value
            }
        }
        ...
        public void SetDiggerUsingToolEffectSize(float newEffectRadius, float newEffectOpacity)
        {
            try
            {
                ...
                {
                    Debug.LogError($"Invalid effect radius: {newEffectRadius}");
                    newEffectRadius = minEffectRadius;
                }

                if (float.IsNaN(newEffectOpacity) || float.IsInfinity(newEffectOpacity))
                {
                    Debug.LogError($"Invalid effect opacity: {newEffectOpacity}");
            ...
            {
                Debug.LogError($"Error in SetDiggerUsingToolEffectSize: {ex.Message}");
            }
        }
        ...
        protected void TriggerDebrisEffect(GameObject debrisEffectPrefab, RaycastHit hit)
        {
            try
            {
                ...
                if (mainCamera == null)
                {
                    Debug.LogError("mainCamera is null for debris effect");
                    return;
                }
            ...
            {
                Debug.LogError($"Error in TriggerDebrisEffect: {ex.Message}");
            }
        }
            ...
            int textureIndex, float opacity, float radius, float height = 100f, bool smooth = true)
        {
            try
            {
                if (digger == null)
                {
                    Debug.LogError("Digger reference is null");
                    return false;
                }
                ...
                {
                    Debug.LogError($"Cannot use invalid position: {position}");
                    return false;
                }
                ...
                {
                    Debug.LogError($"Error in Digger.Modify: {ex.Message}");
                    return false;
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"Error in SafeModify: {ex.Message}");
                return false;
            }
        }
