...
// MyOreNode
        public bool OnHoverStart(GameObject go)
        {
            ...
            // Has this ore TYPE been examined already?
            var alreadyKnown = ExaminationManager.Instance != null &&
                               ExaminationManager.Instance.HasOreBeenExamined(itemTypeMined.ItemID);
            ...
            var nameToShow = alreadyKnown || recognizable ? itemTypeMined.ItemName : itemTypeMined.UnknownName;
            var iconToShow = alreadyKnown || recognizable
                ? itemTypeMined.Icon
                : ExaminationManager.Instance?.defaultUnknownIcon;

            var shortToShow = alreadyKnown || recognizable
                ? itemTypeMined.ShortDescription
                : examinableObjectData.UnknownShortBlurb;
