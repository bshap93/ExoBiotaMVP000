...
namespace Manager
{
    ...
    public class ProgressionManager : Manager, MMEventListener<ProgressionEvent>
    {
        ...
        private void Start()
        {
            ...
            {
                Debug.Log("[ProgressionManager] No save file found, forcing initial save...");
        ...
        private void LoadProgression()
        {
            ...
            if (CollectedObjectives.Count > 0)
                Debug.Log($"Loaded objectives: {CollectedObjectives.Count}");
        }
        ...
        {
            IsNewGame = false;
            Debug.Log("ProgressionManager: Continue game");
        }
        ...
        protected override void LoadBooleanFlags()
        {
            ...
            {
                TutorialFinished = false;
                Debug.Log($"No saved tutorial finished state found. Defaulting to: {TutorialFinished}");
            }
        }
        ...
        public void LoadProgressionObjectivesState()
        {
            ...
            if (ES3.KeyExists(ObjectivesKeyName, _savePath))
            {
                ...
                foreach (var objective in loadedObjectives)
                {
                    ...
                    Debug.Log($"Loaded collectable objective: {objective}");
                }
            }
            else
            {
                ...
                foreach (var key in keys)
                    if (ES3.KeyExists(key, _savePath) && ES3.Load<bool>(key, _savePath))
                    {
                        ...
                        Debug.Log($"Loaded collectable objective: {key}");
                    }
            }
        }
