...
namespace Digger
{
    ...
    public class DiggerRuntimeUsageExample : MonoBehaviour
    {
        ...
        public KeyCode keyToClearScene = KeyCode.R;

        [Header("Display")]
        public bool showModificationResults = true;

        private DiggerMasterRuntime diggerMasterRuntime;
        private ModificationResult lastResult;
        ...
        private void OnModificationComplete(ModificationResult result)
        {
            lastResult = result;
        }
        ...
        private void OnGUI()
        {
            if (!showModificationResults)
                return;

            GUILayout.BeginArea(new Rect(10, 10, 400, 150));
            GUILayout.BeginVertical("box");
            GUILayout.Label("Last Modification Results:", new GUIStyle(GUI.skin.label) { fontStyle = FontStyle.Bold });
            GUILayout.Label($"Removed Matter: {lastResult.RemovedMatterQuantity:F3}");
            GUILayout.Label($"Added Matter: {lastResult.AddedMatterQuantity:F3}");
            GUILayout.Label($"Modified Voxels: {lastResult.TotalModifiedVoxels}");
            GUILayout.Label($"Avg Change/Voxel: {lastResult.AverageChangePerVoxel:F3}");
