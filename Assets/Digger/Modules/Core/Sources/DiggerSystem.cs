...
namespace Digger.Modules.Core.Sources
{
    public class DiggerSystem : MonoBehaviour
    {
...
#if UNITY_EDITOR
            CreateDirs();
            var asset = new TextAsset(DiggerVersion.ToString()) { name = "digger_version" };
            EditorUtils.CreateOrReplaceAsset(asset, GetPathDiggerVersionFile());
#endif
        }
...
#if UNITY_EDITOR
            CreateDirs();
            var asset = new TextAsset(version.ToString()) { name = "current_version" };
            EditorUtils.CreateOrReplaceAsset(asset, GetPathCurrentVersionFile());
        ...
        public async Awaitable<bool> ModifyWithoutMeshes<T>(IOperation<T> operation, bool useBackgroundThreads = false) where T : struct, IJobParallelFor
        {
            ...
            await Modify(operation, area, false, useBackgroundThreads);
        ...
        private async Awaitable BuildMeshes(Dictionary<Vector3i, Chunk> pendingChunks, bool useBackgroundThreads)
        {
            for (int lodIndex = 0; lodIndex < LODCount; ++lodIndex) {
                ...
                foreach (var builtChunk in pendingChunks.Values) {
        ...
        public void PrepareLightmapping()
        {
            if (chunks == null)
                return;
            ...
            var i = 0f;
            foreach (var chunk in chunks) {
                EditorUtility.DisplayProgressBar("Prepare lightmapping", "Generating secondary UV sets...", i / chunks.Count);
                chunk.Value.GenerateSecondaryUVSet();
                i++;
            }
        ...
        public void CreateDirs()
        {
            ...
            if (!Directory.Exists(MeshesPathData))
            {
                Directory.CreateDirectory(MeshesPathData);
            }
#endif
        }
